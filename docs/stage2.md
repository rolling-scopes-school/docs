...
1. В приватном репозитории школы создаём ветку `gh-pages`, удаляем в ней файл `.gitignore`
2. В ветке `gh-pages` создаём папку с именем задания
3. В созданную папку помещаем собранные файлы проекта. Файл `index.html` должен находиться на верхнем уровне
4. Ссылка на задеплоенную версию приложения имеет вид: `https://rolling-scopes-school.github.io/repository-name/folder-name`, где
    - `repository-name` - имя вашего репозитория в rs school  
    - `folder-name` - имя папки с файлами проекта
5. Таким образом деплоятся и все остальные проекты, для каждого из них создаётся отдельная папка 
