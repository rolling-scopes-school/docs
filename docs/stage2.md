...
## Репозиторий
 После выдачи приватных репозиториев школы, по умолчанию, все таски необходимо сохранять в приватный репозиторий школы на GitHub.
...
1. В приватном репозитории школы от ветки `master` создаём ветку `gh-pages`
2. В ветке `gh-pages` необходимо разместить папку с именем задания, а в ней файлы проекта. Файл `index.html` должен находиться на верхнем уровне. Сделать это можно двумя способами:
...
3. Ссылка на задеплоенную версию приложения имеет вид: `https://rolling-scopes-school.github.io/repository-name/folder-name`, где
    - `repository-name` - имя вашего репозитория в rs school  
    - `folder-name` - имя папки с файлами проекта
4. Таким образом деплоятся и все остальные проекты, для каждого из них создаётся отдельная папка 
...
3. Ссылка на задеплоенную версию вашего приложения или сайта. Для деплоя можно использовать:
    ...
    - при отсутствии приватного репозитория школы - [netlify.com](https://app.netlify.com/drop), либо другой подобный хостинг.    
